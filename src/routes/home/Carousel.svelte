  ...
  <div class="carousel-items">
    {#each imgSrcs as src, i}
      <div class="carousel-item" 
        ...
        class:smooth-transform={(i==active || i==lastActive)}
      >
        <img src="home/carousel/{src}" alt="{i} of {imgSrcs.length}">
      </div>
    {/each}
  </div>
  <div class="nav-dots">
    {#each imgSrcs as _, i}
      <button class="nav-dot" aria-label="nav dot" class:active-dot={i === active} onclick={() => handleClick(i)}></button>
    {/each}
  </div>
