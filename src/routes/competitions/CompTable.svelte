<!-- 
  This component makes the table for a list of given type of competitions.
  The type could be upcoming, recent or current.
-->
...
<script>
  ...
  import CompActions from "./CompActions.svelte";
  import CompEvents  from "./CompEvents.svelte"

  // accept props from parent
  let { type, comps } = $props();

  // for each comp, set its registration status text and whether registration is closed or not
  for(let i=0; i<comps.length; i++){
    comps[i].regText = type === "upcoming" ? getRegDate(comps[i]) 
      : "recent" ? "Competition has finished" : null;
